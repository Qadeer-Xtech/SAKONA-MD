...
import config from '../../config.js'; // Ensure config.js has your bot settings

const downloadMediaFireFile = async (m, gss) => {

  const prefix = config.PREFIX; // Get prefix from config.js
  ...
  const caption = args.slice(1).join(' ') || 'SAKONA-MD ʙᴏᴛ'; // Default caption if none provided
